...
for i in $(seq 0 $maxid); do
    ...
    ssh ap-$index.$SUFFIX python3 $BASE/access_point.py $CONFIG \
        $CONFIG/$id.json --no-ping -v $VERBOSE &
done
sleep 2
...
for i in $(seq 0 $maxid); do
    ...
    ssh client-$index.$SUFFIX python3 $BASE/client.py -p $port $CONFIG \
        $CONFIG/$id.json -u manual -a $(($i % $aps)) -m --no-ping -v $VERBOSE &
done
...
for i in $(seq 0 $maxid); do
    ...
    ssh trustee-$index.$SUFFIX python3 $BASE/trustee.py -p $port $CONFIG $CONFIG/$id.json &
done
