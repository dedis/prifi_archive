...
class Verdict:
    def __init__(self, key, public_keys):
        ...
        for idx in range(len(self.public_keys)):
            ...
            if self.index == -1 and ks == self.key.y:
                self.index = idx
                continue
            multi = 1 if self.index == -1 else -1
            add = -1 if self.index == -1 else 0
            self.shared_secret = (self.shared_secret + (pow(ks, self.key.x, p) * multi) % p + add) % q
        assert self.index != -1
    ...
    def generate_ciphertext(self, generator, data = 1):
        return (data * pow(generator, self.shared_secret, p) % p)
...
def main():
    ...
    for idx in range(count):
        if idx == slot_index:
            ciphertexts.append(verdicts[idx].generate_ciphertext(generator, msgi))
...
#        assert verdicts[0].verify(generator, ciphertexts[idx], proofs[idx], slot_index, idx) == True
    ...
    for ciphertext in ciphertexts:
        cleartext = (cleartext * ciphertext) % p
