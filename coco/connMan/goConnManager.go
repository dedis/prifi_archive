 ...
 */
func (gcm *GoConnManager) Init(key abstract.Point, goDir *conn.GoDirectory) *GoConnManager {
	gcm.pubKey = key
	gcm.peerMap = make(map[abstract.Point]*conn.GoConn)
	if goDir == nil {
		gcm.dir = conn.NewGoDirectory()
	} else {
		gcm.dir = goDir
	}
	return gcm
...
func (gcm *GoConnManager) AddConn(theirKey abstract.Point) error {
	newConn, err := conn.NewGoConn(gcm.dir, gcm.pubKey.String(), theirKey.String())
	if err != nil {
		return err
	}
...
func (gcm *GoConnManager) GetDir() *conn.GoDirectory {
	return gcm.dir
}
 ...
 */
func (gcm *GoConnManager) Put(p abstract.Point, data conn.BinaryMarshaler) error {
 ...
 */
func (gcm *GoConnManager) Get(p abstract.Point, bum conn.BinaryUnmarshaler) error {
